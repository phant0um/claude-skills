...
trigger: ["escrever artigo", "rascunho", "redação", "moldar texto", "write it as a journey", "beat by beat", "choose your own adventure article"]
---
...
**Prerequisites settled:** audience knows what a database index is; does not know this team's incident.

**Grounded set:** { database index }

You offer three candidate starting beats:

- **A — cold open on the symptom** ("At 2am the dashboard went red.") · grounds *the incident*.
- **B — the one-line thesis** ("Every index we added made the next one slower.") · grounds *index interaction*, requires only *database index*.
- **C — a definition beat** on write amplification · grounds *write amplification*.

User picks **A**. You write only that beat to the file. Grounded set becomes { database index, the incident }.

Re-reading the file, you offer next beats reachable from that set:

- **A1 — what we saw first** (requires *the incident*) · grounds *write amplification*.
- **A2 — the red herring** (requires *the incident*) · grounds *the failed rollback*.

Picking A1 would then unlock any later beat that was waiting on *write amplification*. You keep looping until the journey lands.
