...
Before asserting a sourced fact, verify it against a primary source; if you can't confirm it, mark the claim `[unverified]` rather than letting it stand. This reinforces grounding: an unverified fact should never become a foundation a later block leans on.

## Exemplo

**Input pile** (excerpt of a fragments file):

```markdown
# Why our deploys got slow

- CI went from 4 min to 22 min over six months, nobody noticed the slope.
- The cache key included the git SHA, so it never hit.
- "A regression with no single commit to blame."
```

**Prerequisites settled with user:** reader knows what CI and a build cache are; does NOT know this team's setup.

**Candidate opening chosen** (grounds the term *slope blindness*):

> No single deploy was slow. That was the problem. Our CI crept from four minutes
> to twenty-two over half a year, one imperceptible step at a time — slope
> blindness, the failure mode where nothing is ever the regression because
> everything is.

**Next block** (may now lean on *slope blindness*; grounds the cache-key cause), pulled and reworked from the second and third fragments — placed as prose because it carries an argument, not parallel items.
